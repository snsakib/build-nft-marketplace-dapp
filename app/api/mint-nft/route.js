...
// Define an asynchronous function to pin a file to IPFS
async function pinToIPFS(url, data, apiType) {
  try {
    ...
    // Log and rethrow any errors that occur during the pinning process
...
export async function POST(request) {
  try {
    ...
    // Create a new FormData object for the NFT file and its metadata
    const fileData = new FormData();
    fileData.append("file", NFT_FILE);
    fileData.append("pinataMetadata", JSON.stringify({ name: NFT_NAME }));

    // Pin the NFT file to IPFS and get the resulting CID
